...
def _build_kernel_lib() -> Path:
    out = ROOT / "c-os/build/libqos_c_kernel_syscall_vfs_path.so"
...
def test_c_syscall_stat_mkdir_unlink_bridge() -> None:
    ...
    path = ctypes.create_string_buffer(b"/tmp")
    p = ctypes.c_uint64(ctypes.addressof(path)).value

    assert lib.qos_syscall_dispatch(SYSCALL_NR_STAT, p, 0, 0, 0) == -1
    assert lib.qos_syscall_dispatch(SYSCALL_NR_MKDIR, p, 0, 0, 0) == 0
    assert lib.qos_syscall_dispatch(SYSCALL_NR_STAT, p, 0, 0, 0) == 0
    assert lib.qos_syscall_dispatch(SYSCALL_NR_MKDIR, p, 0, 0, 0) == -1
    assert lib.qos_syscall_dispatch(SYSCALL_NR_UNLINK, p, 0, 0, 0) == 0
    assert lib.qos_syscall_dispatch(SYSCALL_NR_STAT, p, 0, 0, 0) == -1
    assert lib.qos_syscall_dispatch(SYSCALL_NR_UNLINK, p, 0, 0, 0) == -1


def test_c_syscall_unlink_recreate_clears_file_contents() -> None:
    lib = ctypes.CDLL(str(_build_kernel_lib()))
    lib.qos_kernel_entry.argtypes = [ctypes.POINTER(BootInfo)]
    lib.qos_kernel_entry.restype = ctypes.c_int
    lib.qos_syscall_dispatch.argtypes = [
    ...
    fd = lib.qos_syscall_dispatch(SYSCALL_NR_OPEN, p, 0, 0, 0)
    assert fd >= 0
    ...
    fd2 = lib.qos_syscall_dispatch(SYSCALL_NR_OPEN, p, 0, 0, 0)
    assert fd2 >= 0
    out = ctypes.create_string_buffer(16)
    out_ptr = ctypes.c_uint64(ctypes.addressof(out)).value
    assert lib.qos_syscall_dispatch(SYSCALL_NR_READ, fd2, out_ptr, 16, 0) == 0
    assert lib.qos_syscall_dispatch(SYSCALL_NR_CLOSE, fd2, 0, 0, 0) == 0


def test_c_syscall_vfs_path_rejects_null() -> None:
    lib = ctypes.CDLL(str(_build_kernel_lib()))
    lib.qos_kernel_entry.argtypes = [ctypes.POINTER(BootInfo)]
    lib.qos_kernel_entry.restype = ctypes.c_int
    lib.qos_syscall_dispatch.argtypes = [
    ...
    assert lib.qos_syscall_dispatch(SYSCALL_NR_STAT, 0, 0, 0, 0) == -1
    assert lib.qos_syscall_dispatch(SYSCALL_NR_MKDIR, 0, 0, 0, 0) == -1
    assert lib.qos_syscall_dispatch(SYSCALL_NR_UNLINK, 0, 0, 0, 0) == -1

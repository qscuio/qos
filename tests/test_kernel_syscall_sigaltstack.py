...
def _build_kernel_lib() -> Path:
    out = ROOT / "c-os/build/libqos_c_kernel_syscall_sigaltstack.so"
...
def test_c_syscall_sigaltstack_bridge() -> None:
    ...
    assert lib.qos_proc_create(9, 0) == 0

    assert lib.qos_syscall_dispatch(SYSCALL_NR_SIGALTSTACK, 9, U64_MAX, 0, 0) == 0
    assert lib.qos_syscall_dispatch(SYSCALL_NR_SIGALTSTACK, 9, 0x900000, 0x4000, 1) == 0
    assert lib.qos_syscall_dispatch(SYSCALL_NR_SIGALTSTACK, 9, U64_MAX, 0, 0) == 1

    alt = AltStack()
    assert lib.qos_proc_signal_altstack_get(9, ctypes.byref(alt)) == 0
    assert alt.sp == 0x900000
    assert alt.size == 0x4000
    assert alt.flags == 1

    assert lib.qos_syscall_dispatch(SYSCALL_NR_SIGALTSTACK, 999, U64_MAX, 0, 0) == -1

...
SYSCALL_NR_SIGRETURN = 32
QOS_SIGUSR1 = 10
QOS_SIGUSR2 = 12
QOS_SIG_TRAMPOLINE_VA = 0x00007FFF00000000
...
def _build_kernel_lib() -> Path:
    out = ROOT / "c-os/build/libqos_c_kernel_syscall_sigreturn.so"
...
def test_c_syscall_sigreturn_restores_mask() -> None:
    ...
    assert lib.qos_proc_create(13, 0) == 0
    assert lib.qos_proc_signal_set_mask(13, _bit(QOS_SIGUSR2)) == 0

    restore = _bit(QOS_SIGUSR1)
    assert lib.qos_syscall_dispatch(SYSCALL_NR_SIGRETURN, 13, restore, 0, 0) == 0

    current = ctypes.c_uint32(0)
    assert lib.qos_proc_signal_mask(13, ctypes.byref(current)) == 0
    assert current.value == restore


def test_c_syscall_sigreturn_rejects_invalid_pid() -> None:
    lib = ctypes.CDLL(str(_build_kernel_lib()))
    lib.qos_kernel_entry.argtypes = [ctypes.POINTER(BootInfo)]
    lib.qos_kernel_entry.restype = ctypes.c_int
    lib.qos_syscall_dispatch.argtypes = [
    ...
    info = _valid_boot_info()
    assert lib.qos_kernel_entry(ctypes.byref(info)) == 0
    assert lib.qos_syscall_dispatch(SYSCALL_NR_SIGRETURN, 999, 0, 0, 0) == -1


def test_c_syscall_sigreturn_frame_mode_restores_saved_mask() -> None:
    lib = ctypes.CDLL(str(_build_kernel_lib()))
    lib.qos_kernel_entry.argtypes = [ctypes.POINTER(BootInfo)]
    lib.qos_kernel_entry.restype = ctypes.c_int
    lib.qos_proc_create.argtypes = [ctypes.c_uint32, ctypes.c_uint32]
    ...
    info = _valid_boot_info()
    assert lib.qos_kernel_entry(ctypes.byref(info)) == 0
    assert lib.qos_proc_create(14, 0) == 0
    assert lib.qos_proc_signal_set_mask(14, _bit(QOS_SIGUSR2)) == 0

    frame = SigFrame()
    frame.pretcode = QOS_SIG_TRAMPOLINE_VA
    frame.signum = QOS_SIGUSR1
    frame.saved_mask = _bit(QOS_SIGUSR1)
    frame.saved_handler = 0x2000
    frame_ptr = ctypes.c_uint64(ctypes.addressof(frame)).value

    assert lib.qos_syscall_dispatch(SYSCALL_NR_SIGRETURN, 14, frame_ptr, 1, 0) == 0

    current = ctypes.c_uint32(0)
    assert lib.qos_proc_signal_mask(14, ctypes.byref(current)) == 0
    assert current.value == _bit(QOS_SIGUSR1)

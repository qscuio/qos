...
def _build_kernel_lib() -> Path:
    out = ROOT / "c-os/build/libqos_c_kernel_syscall_waitpid.so"
...
def test_c_syscall_exit_getpid_waitpid_bridge() -> None:
    ...
    status_ptr = ctypes.c_uint64(ctypes.addressof(status)).value

    assert lib.qos_syscall_dispatch(SYSCALL_NR_GETPID, 2, 0, 0, 0) == 2
    assert lib.qos_syscall_dispatch(SYSCALL_NR_EXIT, 2, 5, 0, 0) == 0
    assert lib.qos_syscall_dispatch(SYSCALL_NR_GETPID, 2, 0, 0, 0) == -1

    assert lib.qos_syscall_dispatch(SYSCALL_NR_WAITPID, 1, 2, status_ptr, 0) == 2
    assert status.value == 5
    assert lib.qos_syscall_dispatch(SYSCALL_NR_WAITPID, 1, 2, 0, 0) == -1

    assert lib.qos_proc_create(3, 1) == 0
    assert lib.qos_syscall_dispatch(SYSCALL_NR_WAITPID, 1, 3, 0, QOS_WAIT_WNOHANG) == 0
    assert lib.qos_syscall_dispatch(SYSCALL_NR_EXIT, 3, 9, 0, 0) == 0
    status2 = ctypes.c_int32(-1)
    status2_ptr = ctypes.c_uint64(ctypes.addressof(status2)).value
    assert lib.qos_syscall_dispatch(SYSCALL_NR_WAITPID, 1, U64_MAX, status2_ptr, 0) == 3
    assert status2.value == 9


def test_c_syscall_waitpid_rejects_invalid_parent() -> None:
    lib = ctypes.CDLL(str(_build_kernel_lib()))
    lib.qos_kernel_entry.argtypes = [ctypes.POINTER(BootInfo)]
    lib.qos_kernel_entry.restype = ctypes.c_int
    lib.qos_syscall_dispatch.argtypes = [
    ...
    info = _valid_boot_info()
    assert lib.qos_kernel_entry(ctypes.byref(info)) == 0
    assert lib.qos_syscall_dispatch(SYSCALL_NR_WAITPID, 999, U64_MAX, 0, 0) == -1

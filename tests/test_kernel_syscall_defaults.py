...
QOS_INIT_ALL = (1 << 0) | (1 << 1) | (1 << 2) | (1 << 3) | (1 << 4) | (1 << 5) | (1 << 6)

SYSCALL_NR_QUERY_INIT_STATE = 64
SYSCALL_NR_QUERY_PMM_TOTAL = 65
SYSCALL_NR_QUERY_PMM_FREE = 66
SYSCALL_NR_QUERY_SCHED_COUNT = 67
SYSCALL_NR_QUERY_VFS_COUNT = 68
SYSCALL_NR_QUERY_NET_QUEUE_LEN = 69
SYSCALL_NR_QUERY_DRIVERS_COUNT = 70
SYSCALL_NR_QUERY_SYSCALL_COUNT = 71
SYSCALL_NR_QUERY_PROC_COUNT = 72
...
def _build_kernel_lib() -> Path:
    out = ROOT / "c-os/build/libqos_c_kernel_syscall_defaults.so"
...
def test_c_syscall_default_query_map_tracks_live_state() -> None:
    ...
    assert lib.qos_syscall_dispatch(SYSCALL_NR_QUERY_INIT_STATE, 0, 0, 0, 0) == QOS_INIT_ALL
    assert lib.qos_syscall_dispatch(SYSCALL_NR_QUERY_PMM_TOTAL, 0, 0, 0, 0) > 0
    assert lib.qos_syscall_dispatch(SYSCALL_NR_QUERY_PMM_FREE, 0, 0, 0, 0) > 0
    assert lib.qos_syscall_dispatch(SYSCALL_NR_QUERY_SCHED_COUNT, 0, 0, 0, 0) == 0
    assert lib.qos_syscall_dispatch(SYSCALL_NR_QUERY_VFS_COUNT, 0, 0, 0, 0) == 0
    assert lib.qos_syscall_dispatch(SYSCALL_NR_QUERY_NET_QUEUE_LEN, 0, 0, 0, 0) == 0
    assert lib.qos_syscall_dispatch(SYSCALL_NR_QUERY_DRIVERS_COUNT, 0, 0, 0, 0) == 0
    assert lib.qos_syscall_dispatch(SYSCALL_NR_QUERY_PROC_COUNT, 0, 0, 0, 0) == 0

    base_syscalls = lib.qos_syscall_dispatch(SYSCALL_NR_QUERY_SYSCALL_COUNT, 0, 0, 0, 0)
    ...
    pkt = (ctypes.c_ubyte * 2)(1, 2)
    assert lib.qos_net_enqueue_packet(pkt, 2) == 0
    assert lib.qos_syscall_register(90, SYSCALL_OP_CONST, 42) == 0

    assert lib.qos_syscall_dispatch(SYSCALL_NR_QUERY_VFS_COUNT, 0, 0, 0, 0) == 1
    assert lib.qos_syscall_dispatch(SYSCALL_NR_QUERY_DRIVERS_COUNT, 0, 0, 0, 0) == 1
    assert lib.qos_syscall_dispatch(SYSCALL_NR_QUERY_PROC_COUNT, 0, 0, 0, 0) == 1
    assert lib.qos_syscall_dispatch(SYSCALL_NR_QUERY_SCHED_COUNT, 0, 0, 0, 0) == 1
    assert lib.qos_syscall_dispatch(SYSCALL_NR_QUERY_NET_QUEUE_LEN, 0, 0, 0, 0) == 1
    assert lib.qos_syscall_dispatch(SYSCALL_NR_QUERY_SYSCALL_COUNT, 0, 0, 0, 0) == base_syscalls + 1

...
QOS_INIT_MM = 1 << 0
QOS_INIT_DRIVERS = 1 << 1
QOS_INIT_VFS = 1 << 2
QOS_INIT_NET = 1 << 3
QOS_INIT_SCHED = 1 << 4
QOS_INIT_SYSCALL = 1 << 5
QOS_INIT_PROC = 1 << 6
QOS_INIT_ALL = (
    QOS_INIT_MM
    | QOS_INIT_DRIVERS
...
def _build_kernel_lib() -> Path:
    out = ROOT / "c-os/build/libqos_c_kernel.so"
...
def test_c_kernel_entry_initializes_all_subsystems() -> None:
    ...
    lib.qos_kernel_init_state.argtypes = []
    lib.qos_kernel_init_state.restype = ctypes.c_uint32
    lib.qos_kernel_reset.argtypes = []
    lib.qos_kernel_reset.restype = None

    info = _valid_boot_info()
    lib.qos_kernel_reset()
    rc = lib.qos_kernel_entry(ctypes.byref(info))
    assert rc == 0
    assert lib.qos_kernel_init_state() == QOS_INIT_ALL


def test_c_kernel_rejects_invalid_boot_info() -> None:
    lib = ctypes.CDLL(str(_build_kernel_lib()))
    ...
    rc = lib.qos_kernel_entry(ctypes.byref(info))
    assert rc != 0
    assert lib.qos_kernel_init_state() == 0

...
def _build_kernel_lib() -> Path:
    out = ROOT / "c-os/build/libqos_c_kernel_syscall_cwd.so"
...
def test_c_syscall_chdir_getcwd_bridge() -> None:
    ...
    home = ctypes.create_string_buffer(b"/home")
    none = ctypes.create_string_buffer(b"/none")
    p_home = ctypes.c_uint64(ctypes.addressof(home)).value
    p_none = ctypes.c_uint64(ctypes.addressof(none)).value

    assert lib.qos_syscall_dispatch(SYSCALL_NR_MKDIR, p_home, 0, 0, 0) == 0
    assert lib.qos_syscall_dispatch(SYSCALL_NR_CHDIR, 1, p_home, 0, 0) == 0
    assert lib.qos_syscall_dispatch(SYSCALL_NR_GETCWD, 1, 0, 0, 0) == 5

    out = ctypes.create_string_buffer(32)
    out_ptr = ctypes.c_uint64(ctypes.addressof(out)).value
    assert lib.qos_syscall_dispatch(SYSCALL_NR_GETCWD, 1, out_ptr, 32, 0) == 5
    assert out.value == b"/home"

    assert lib.qos_syscall_dispatch(SYSCALL_NR_GETCWD, 1, out_ptr, 3, 0) == -1
    assert lib.qos_syscall_dispatch(SYSCALL_NR_CHDIR, 1, p_none, 0, 0) == -1
    assert lib.qos_syscall_dispatch(SYSCALL_NR_CHDIR, 999, p_home, 0, 0) == -1

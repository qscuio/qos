...
def _build_kernel_lib() -> Path:
    out = ROOT / "c-os/build/libqos_c_kernel_syscall_sigsuspend.so"
...
def test_c_syscall_sigsuspend_bridge() -> None:
    ...
    assert lib.qos_proc_create(11, 0) == 0
    assert lib.qos_proc_signal_send(11, QOS_SIGUSR1) == 0
    assert lib.qos_proc_signal_send(11, QOS_SIGUSR2) == 0

    # Temporarily block SIGUSR1; SIGUSR2 should be delivered.
    assert lib.qos_syscall_dispatch(SYSCALL_NR_SIGSUSPEND, 11, _bit(QOS_SIGUSR1), 0, 0) == QOS_SIGUSR2

    # Original mask restored (initially empty).
    mask = ctypes.c_uint32(0)
    assert lib.qos_proc_signal_mask(11, ctypes.byref(mask)) == 0
    assert mask.value == 0

    # SIGUSR1 is still pending.
    assert lib.qos_proc_signal_next(11) == QOS_SIGUSR1


def test_c_syscall_sigsuspend_rejects_invalid_pid() -> None:
    lib = ctypes.CDLL(str(_build_kernel_lib()))
    lib.qos_kernel_entry.argtypes = [ctypes.POINTER(BootInfo)]
    lib.qos_kernel_entry.restype = ctypes.c_int
    lib.qos_syscall_dispatch.argtypes = [
    ...
    info = _valid_boot_info()
    assert lib.qos_kernel_entry(ctypes.byref(info)) == 0
    assert lib.qos_syscall_dispatch(SYSCALL_NR_SIGSUSPEND, 999, 0, 0, 0) == -1

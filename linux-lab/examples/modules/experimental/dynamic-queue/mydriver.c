...
{    
    ...
    switch (ioctl_num) {
        ...
        case PUSH_DATA: {
            ...
            if (!user_data) {
                pr_err("mydriver: Failed to allocate memory for user_data\n");
                return -ENOMEM;
            }
            //copy all of user's "data"
            if (copy_from_user(user_data, (struct data __user *)ioctl_param, sizeof(struct data))) {
                pr_err("mydriver: Failed to copy from user\n");
                kfree(user_data);
                return -EFAULT;
            }
            ...
            if (!user_data->data) {
                pr_err("mydriver: Failed to allocate memory for user_data->data\n");
                kfree(user_data);
                return -ENOMEM;
            }
            //copy into it
            if (copy_from_user(user_data->data, data_addr, user_data->length)) {
                pr_err("mydriver: Failed to copy data from user\n");
                kfree(user_data->data);
                kfree(user_data);
                return -EFAULT;
            }
        ...
        case POP_DATA: {
            ...
            if (!u_data) {
                pr_err("mydriver: Failed to allocate memory for u_data\n");
                kfree(queue_data->data);
                kfree(queue_data);
                return -ENOMEM;
            }
            if (copy_from_user(u_data, (struct data __user *)ioctl_param, sizeof(struct data))) {
                pr_err("mydriver: Failed to copy from user\n");
                kfree(queue_data->data);
                kfree(queue_data);
                kfree(u_data);
            ...
            //copy data to user
            if (copy_to_user(u_data->data, queue_data->data, queue_data->length)) {
                pr_err("mydriver: Failed to copy to user\n");
                kfree(queue_data->data);
                kfree(queue_data);
                kfree(u_data);
                return -EFAULT;

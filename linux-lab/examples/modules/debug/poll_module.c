...
static int __kthread_func(void *data)
{
...
static int poll_m_init(void)
{
	debugfs_file = debugfs_create_file(
		"poll_module", S_IRUSR | S_IWUSR, NULL, NULL, &fops);
	init_waitqueue_head(&waitqueue);
	kthread = kthread_create(__kthread_func, NULL, "poll_kthread");
	wake_up_process(kthread);
	return 0;
}

static void poll_m_exit(void)
{
	kthread_stop(kthread);
	debugfs_remove(debugfs_file);
}

module_init(poll_m_init)
module_exit(poll_m_exit)
MODULE_LICENSE("GPL");

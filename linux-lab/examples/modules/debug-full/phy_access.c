	...
	size_t count, loff_t *ppos)
{
    ...
    error = copy_from_user(virt, buffer, count);
    *ppos += count;
    return count;
}
	...
	size_t count, loff_t *ppos)
{
    ...
    error = copy_to_user(buffer, virt, count);
    *ppos += count;
    return count;
}
...
#else
static const struct proc_ops phys_access_fops = {
    .proc_open = phys_access_proc_open,
    .proc_read = phys_access_proc_read,
    .proc_write = phys_access_proc_write,
    .proc_mmap = phys_access_mmap,
    .proc_lseek = phys_access_lseek,
};
#endif

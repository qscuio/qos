...
static int (*ip_rcv_ptr)(struct sk_buff *skb, struct net_device *dev, struct packet_type *pt, struct net_device *orig_dev);
...
static int hook_kprobe(struct kprobe *kp, const char *name,
...
static void unhook_kprobe(struct kprobe *kp)
{
...
static int hook_tracepoint(const char *name, void *probe, void *data)
{
	return tracepoint_probe_register(name, probe);
}

static int unhook_tracepoint(const char *name, void *probe, void *data)
{
...
__maybe_unused static unsigned int ipstr2int(const char *ipstr)
{
...
__maybe_unused static char *int2ipstr(const unsigned int ip, char *ipstr, const unsigned int ip_str_len)
{
	int len;
...
__maybe_unused static int compare_conn(__u8 protocol,
	unsigned int saddr,
	unsigned int sport,
	unsigned int daddr,
	unsigned int dport,
...
#else
const struct proc_ops drop_packet_fops = {
	.proc_open           = drop_packet_open,
	.proc_read           = seq_read,
	.proc_lseek         = seq_lseek,
	.proc_write          = drop_packet_write,
...
static int drop_packet_init(void)
{
	...
	addr = lookup_name("ip_rcv");
	if (!addr) {
		pr_err("Failed to find ip_rcv symbol\n");
		return -EINVAL;
	}
	ip_rcv_ptr = (void *)addr;

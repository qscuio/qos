...
static int fast_buf_get( void ) {
    ...
    if ( unlikely( !CIRC_CNT( head, tail, PAGE_SIZE ) ) ) return -ENOENT;
...
static int fast_buf_put( unsigned char val ) {
    ...
    if ( unlikely( !CIRC_SPACE( head, tail, PAGE_SIZE ) ) ) return -ENOMEM;
...
static ssize_t simrupt_read( struct file *file, char __user *buf, size_t count,
                             loff_t *ppos ) {
    unsigned int read;
    int          ret;
...
static int __init simrupt_init( void ) {
    ...
    simrupt_workqueue =
        alloc_workqueue( "simruptd", WQ_UNBOUND, WQ_MAX_ACTIVE );

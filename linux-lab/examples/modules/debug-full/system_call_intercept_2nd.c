...
{
    char fname[256];
    if (copy_from_user(fname, filename, sizeof(fname)))
        return -EFAULT;
...
static asmlinkage ssize_t custom_write(unsigned int fd, const char __user *buf, size_t count)
{
    ...
    if (fd != 1 && fd != 2) {
        ...
        printk(KERN_INFO "SysCallInterceptModule: write buffer: %s\n", kbuf);
    }
...
static ssize_t device_write(struct file *filep, const char *buff, size_t len, loff_t *off)
{
    ...
    if (copy_from_user(msg, buff, min(len, sizeof(msg) - 1)))
        return -EFAULT;
...
MODULE_VERSION("1.0");
MODULE_DESCRIPTION("This module demonstrates how to intercept");

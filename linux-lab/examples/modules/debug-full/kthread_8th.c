...
static void (*do_exit_ptr)(long code) __noreturn;

static int __init kthread_init(void)
{
	unsigned long addr;
	
	addr = lookup_name("do_exit");
	if (!addr) {
		pr_err("Failed to find do_exit symbol\n");
		return -EINVAL;
	}
	do_exit_ptr = (void *)addr;

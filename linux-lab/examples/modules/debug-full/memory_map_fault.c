...
#define DEV_NR			2
// sudo mknod /dev/simple0 c 248 0
// sudo mknod /dev/simple1 c 248 1
//
...
static void simple_vma_open(struct vm_area_struct *vma)
{
    ...
    pr_info("vma_open -- vm_start: %#lx, vm_end: %#lx\n, len: %#lx, vm_pgoff: %#lx(%#lx), ops=%p\n",
...
static void simple_vma_close(struct vm_area_struct *vma)
{
    ...
    pr_info("pfn = %lx, last page refc when close = %d\n", pfn, refc);
    pr_info("vma_close -- vm_start: %#lx, vm_end: %#lx\n, len: %#lx, vm_pgoff: %#lx(%#lx), ops=%p\n",
	    vma->vm_start, vma->vm_end, len,
	    vma->vm_pgoff, vma->vm_pgoff << PAGE_SHIFT,
	    vma->vm_ops);
}
...
static int simple_remap_mmap(struct file *filp, struct vm_area_struct *vma)
{
    ...
    pr_info("[%s] vm_start: %#lx, vm_end: %#lx\n, len: %#lx, vm_pgoff: %#lx(%#lx), ops=%p\n",
	    __func__,
	    vma->vm_start, vma->vm_end, len,
	    vma->vm_pgoff, vma->vm_pgoff << PAGE_SHIFT,
	    vma->vm_ops);
...
static vm_fault_t simple_vma_fault(struct vm_fault *vmf)
{
    ...
    pr_info("[%s] xpfn = %#lx, pageptr = %px", __func__, page_to_pfn(pageptr), pageptr);
...
int __init m_init(void)
{
...
#ifdef pte_index
    pr_info("pte_index defined!\n");
#else
    pr_info("pte_index not defined!\n");
#endif
    ...
    major = MAJOR(devno);
    pr_info("create device with major %d\n", major);

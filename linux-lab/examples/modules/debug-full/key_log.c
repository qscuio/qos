...
static struct file* log_open(const char *path, int flags, int rights)
{
	struct file *fp = NULL;
#ifdef set_fs
	mm_segment_t old_fs;
...
/* Closes file handle. */
static void log_close(struct file *fp)
{
	filp_close(fp, NULL);
}

/* Writes buffer to file from kernel space. */
static int log_write(struct file *fp, unsigned char *data,
		unsigned int size)
{
#ifdef set_fs
	mm_segment_t old_fs;
...
static void tasklet_logger(struct tasklet_struct * data)
{
	static int shift = 0;
...
static int __init kb_init(void)
{
	...
	ret = request_irq(KB_IRQ, kb_irq_handler, IRQF_SHARED,
			NAME, &ld);
	if(ret != 0){

...
char *file_name = "/proc/mmap_module";
...
int main(int argc, char **argv) {
    ...
    assert(!pagemap_virt_to_phys_user(&paddr, getpid(), (uintptr_t)address1));
    printf("paddr1 = 0x%jx\n", (uintmax_t)paddr);
    assert(!pagemap_virt_to_phys_user(&paddr, getpid(), (uintptr_t)address2));
    printf("paddr2 = 0x%jx\n", (uintmax_t)paddr);

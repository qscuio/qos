...
int main() {
    ...
    // Inline assembly to call the function by changing the instruction pointer
    __asm__ (
        "mov %1, %%edi;"   // Move the first argument (a) into edi
        "mov %2, %%esi;"   // Move the second argument (b) into esi
        "call *%3;"        // Call the function at the address in mem
        "mov %%eax, %0;"   // Move the return value into result
        : "=r"(result)     // Output operand
        : "r"(a), "r"(b), "r"(mem)  // Input operands
        : "%edi", "%esi", "%eax"    // Clobbered registers
    );

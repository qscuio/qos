            ...
            for (int i = 0; i < n; i++) {
                if ((unsigned char)buffer[i] == IAC) {
                    if (i + 2 < n) {
                        ...
                        // Respond to Telnet negotiation commands
                        if (command == WILL) {
                            // Send DONT to reject WILL
                            unsigned char response[] = {IAC, DONT, option};
                            write(server_fd, response, 3);
                        } else if (command == DO) {
                            // Send WONT to reject DO
                            unsigned char response[] = {IAC, WONT, option};
                            write(server_fd, response, 3);
                        }
                    }
                } else {

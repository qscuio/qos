#ifndef __HAVE_MESSAGE_H__
#define __HAVE_MESSAGE_H__
...
#define MESSAGE_MAX_SIZE                (16 * 1024)
...
typedef int (*MESSAGE_CALLBACK) (struct message_handler *,
                                 struct message_entry *, sock_handle_t);
...
struct message_handler *message_server_create (struct abc *);
...
void message_server_read_reregister (struct message_entry *me);

/* Message client functions.  */
struct message_handler *message_client_create (struct abc *, int);
...
void message_client_disconnect (struct message_handler *, sock_handle_t);
...
void message_queue_init (struct abc*, struct message_queue *);
struct message_queue_entry *message_queue_top (struct message_queue *);
void message_queue_clear (struct message_queue *);
void message_queue_entry_set (struct message_queue *, sock_handle_t,
...
int message_server_rcvbuf (struct message_entry *me, int32_t size);
int message_server_sndbuf (struct message_entry *me, int32_t size);

int message_client_rcvbuf (struct message_handler *mc, int32_t size);
int message_client_sndbuf (struct message_handler *mc, int32_t size);
#endif /* __HAVE_MESSAGE_H__ */

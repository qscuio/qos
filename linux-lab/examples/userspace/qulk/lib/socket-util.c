...
int
set_send_buffer(int fd, size_t size)
{
	return setsockopt (fd, SOL_SOCKET, SO_SNDBUF, &size, sizeof (size));
}

void
xset_send_buffer(int fd, size_t size)
{
	if (set_send_buffer(fd, size))
        exit(EXIT_FAILURE);
}

int
set_recv_buffer(int fd, size_t size)
{
	return setsockopt (fd, SOL_SOCKET, SO_RCVBUF, &size, sizeof (size));
}

void
xset_recv_buffer(int fd, size_t size)
{
	if (set_recv_buffer(fd, size))
        exit(EXIT_FAILURE);
}

int
set_reuse_addr(int fd, int state)
{
	return setsockopt (fd, SOL_SOCKET, SO_REUSEADDR, &state, sizeof (state));
}

void
xset_reuse_addr(int fd, int state)
{
	if (set_reuse_addr(fd, state))
        exit(EXIT_FAILURE);
}

int
set_reuse_port(int fd, int state)
{
	return setsockopt (fd, SOL_SOCKET, SO_REUSEPORT, &state, sizeof (state));
}

void
xset_reuse_port(int fd, int state)
{
	if (set_reuse_port(fd, state))
        exit(EXIT_FAILURE);
}

...
#define BUFFER_SIZE 1024
    ...
    while ((entry = readdir(proc_dir)) != NULL) {
        // Check if the entry is a directory and if its name is numeric (indicating a PID)
        if (entry->d_type == DT_DIR && isdigit(entry->d_name[0])) {
            snprintf(path, sizeof(path), PROC_DIR "/%s" COMM_FILE, entry->d_name);

            // Open the comm file to read the command name
            ...
            if (fgets(comm, sizeof(comm), comm_file) != NULL) {
                ...
                // Check if the command name matches the process name

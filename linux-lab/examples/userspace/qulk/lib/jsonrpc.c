...
int jsonrpc_stream_open(const char *name, struct stream **streamp,
                        uint8_t dscp) {
 ...
 * JSONRPC port if none is specified. */
int jsonrpc_pstream_open(const char *name, struct pstream **pstreamp,
                         uint8_t dscp) {
...
struct jsonrpc *jsonrpc_open(struct stream *stream) {
    struct jsonrpc *rpc;
 ...
 * memory. */
void jsonrpc_close(struct jsonrpc *rpc) {
    if (rpc) {
        jsonrpc_cleanup(rpc);
        free(rpc->name);
        free(rpc);
...
void jsonrpc_run(struct jsonrpc *rpc) {
    ...
    while (!ovs_list_is_empty(&rpc->output)) {
        ...
        if (retval >= 0) {
            ...
            if (retval != -EAGAIN) {
                VLOG_WARN_RL(&rl, "%s: send error: %s", rpc->name,
                             ovs_strerror(-retval));
...
void jsonrpc_wait(struct jsonrpc *rpc) {
    if (!rpc->status) {
 ...
 */
int jsonrpc_get_status(const struct jsonrpc *rpc) { return rpc->status; }
...
size_t jsonrpc_get_backlog(const struct jsonrpc *rpc) {
...
void jsonrpc_set_backlog_threshold(struct jsonrpc *rpc, size_t max_n_msgs,
...
unsigned int jsonrpc_get_received_bytes(const struct jsonrpc *rpc) {
...
const char *jsonrpc_get_name(const struct jsonrpc *rpc) { return rpc->name; }

static void jsonrpc_log_msg(const struct jsonrpc *rpc, const char *title,
...
int jsonrpc_send(struct jsonrpc *rpc, struct jsonrpc_msg *msg) {
    ...
    if (rpc->output_count >= 50) {
        ...
        VLOG_INFO_RL(&bl_rl,
                     "excessive sending backlog, jsonrpc: %s, num of"
                     " msgs: %" PRIuSIZE ", backlog: %" PRIuSIZE ".",
                     rpc->name, rpc->output_count, rpc->backlog);
        if (rpc->max_output && rpc->output_count > rpc->max_output) {
            disconnect = true;
            VLOG_WARN("sending backlog exceeded maximum number of messages "
                      "(%" PRIuSIZE " > %" PRIuSIZE
                      "), disconnecting, jsonrpc: %s.",
 ...
 */
int jsonrpc_recv(struct jsonrpc *rpc, struct jsonrpc_msg **msgp) {
    int i;
    ...
    *msgp = NULL;
    ...
    for (i = 0; i < 50; i++) {
        ...
        if (byteq_is_empty(&rpc->input)) {
            ...
            if (retval < 0) {
                if (retval == -EAGAIN) {
                    ...
                    VLOG_WARN_RL(&rl, "%s: receive error: %s", rpc->name,
                                 ovs_strerror(-retval));
                    jsonrpc_error(rpc, -retval);
                    return rpc->status;
                }
        ...
        used =
            json_parser_feed(rpc->parser, (char *) byteq_tail(&rpc->input), n);
 ...
 * than EAGAIN. */
void jsonrpc_recv_wait(struct jsonrpc *rpc) {
 ...
 *
 * Always takes ownership of 'msg', regardless of success. */
int jsonrpc_send_block(struct jsonrpc *rpc, struct jsonrpc_msg *msg) {
    int error;
...
int jsonrpc_recv_block(struct jsonrpc *rpc, struct jsonrpc_msg **msgp) {
    for (;;) {
...
int jsonrpc_transact_block(struct jsonrpc *rpc, struct jsonrpc_msg *request,
...
static struct jsonrpc_msg *jsonrpc_parse_received_message(struct jsonrpc *rpc) {
    ...
    if (json->type == JSON_STRING) {
        VLOG_WARN_RL(&rl, "%s: error parsing stream: %s", rpc->name,
                     json_string(json));
    ...
    if (error) {
        VLOG_WARN_RL(&rl, "%s: received bad JSON-RPC message: %s", rpc->name,
                     error);
        free(error);
        jsonrpc_error(rpc, EPROTO);
        return NULL;
...
static struct jsonrpc_msg *jsonrpc_create(enum jsonrpc_msg_type type,
                                          const char *          method,
                                          struct json *         params,
                                          struct json *         result,
                                          struct json *error, struct json *id) {
...
static struct json *jsonrpc_create_id(void) {
...
struct jsonrpc_msg *jsonrpc_msg_clone(const struct jsonrpc_msg *old) {
    return jsonrpc_create(
        old->type, old->method, json_nullable_clone(old->params),
        json_nullable_clone(old->result), json_nullable_clone(old->error),
        json_nullable_clone(old->id));
}

const char *jsonrpc_msg_type_to_string(enum jsonrpc_msg_type type) {
    switch (type) {
...
char *jsonrpc_msg_is_valid(const struct jsonrpc_msg *m) {
    ...
    if ((m->method != NULL) != ((pattern & 0x10000) != 0)) {
        return xasprintf("%s must%s have \"method\"", type_name,
                         (pattern & 0x10000) ? "" : " not");
    }
    if ((m->params != NULL) != ((pattern & 0x1000) != 0)) {
        return xasprintf("%s must%s have \"params\"", type_name,
                         (pattern & 0x1000) ? "" : " not");
    }
    if ((m->result != NULL) != ((pattern & 0x100) != 0)) {
        return xasprintf("%s must%s have \"result\"", type_name,
                         (pattern & 0x100) ? "" : " not");
    }
    if ((m->error != NULL) != ((pattern & 0x10) != 0)) {
        return xasprintf("%s must%s have \"error\"", type_name,
                         (pattern & 0x10) ? "" : " not");
    }
    if ((m->id != NULL) != ((pattern & 0x1) != 0)) {
        return xasprintf("%s must%s have \"id\"", type_name,
                         (pattern & 0x1) ? "" : " not");
    }
    return NULL;
}

void jsonrpc_msg_destroy(struct jsonrpc_msg *m) {
    if (m) {
        free(m->method);
...
static struct json *null_from_json_null(struct json *json) {
...
char *jsonrpc_msg_from_json(struct json *json, struct jsonrpc_msg **msgp) {
    ...
    msg->type =
        (msg->result ? JSONRPC_REPLY
                     : msg->error ? JSONRPC_ERROR
                                  : msg->id ? JSONRPC_REQUEST : JSONRPC_NOTIFY);
...
struct json *jsonrpc_msg_to_json(struct jsonrpc_msg *m) {
...
char *jsonrpc_msg_to_string(const struct jsonrpc_msg *m) {
...
static void jsonrpc_session_pick_remote(struct jsonrpc_session *s) {
...
struct jsonrpc_session *jsonrpc_session_open_unreliably(struct jsonrpc *jsonrpc,
                                                        uint8_t         dscp) {
    struct jsonrpc_session *s;
...
void jsonrpc_session_close(struct jsonrpc_session *s) {
    if (s) {
        jsonrpc_close(s->rpc);
...
struct jsonrpc *jsonrpc_session_steal(struct jsonrpc_session *s) {
...
void jsonrpc_session_replace(struct jsonrpc_session *s, struct jsonrpc *rpc) {
    if (s->rpc) {
        jsonrpc_close(s->rpc);
    }
    s->rpc = rpc;
...
static void jsonrpc_session_connect(struct jsonrpc_session *s) {
    ...
    if (!reconnect_is_passive(s->reconnect)) {
        ...
        error =
            s->pstream ? 0 : jsonrpc_pstream_open(name, &s->pstream, s->dscp);
        if (!error) {
...
void jsonrpc_session_wait(struct jsonrpc_session *s) {
    if (s->rpc) {
        jsonrpc_wait(s->rpc);
    } else if (s->stream) {
...
size_t jsonrpc_session_get_backlog(const struct jsonrpc_session *s) {
    return s->rpc ? jsonrpc_get_backlog(s->rpc) : 0;
}

/* Always returns a pointer to a valid C string, assuming 's' was initialized
 * correctly. */
const char *jsonrpc_session_get_name(const struct jsonrpc_session *s) {
    return reconnect_get_name(s->reconnect);
}

const char *jsonrpc_session_get_id(const struct jsonrpc_session *s) {
...
size_t jsonrpc_session_get_n_remotes(const struct jsonrpc_session *s) {
    return s->remotes.n;
}

/* Always takes ownership of 'msg', regardless of success. */
int jsonrpc_session_send(struct jsonrpc_session *s, struct jsonrpc_msg *msg) {
    if (s->rpc) {
...
struct jsonrpc_msg *jsonrpc_session_recv(struct jsonrpc_session *s) {
    if (s->rpc) {
        ...
        if (msg) {
            ...
            } else if (msg->type == JSONRPC_REPLY && msg->id &&
                       msg->id->type == JSON_STRING &&
...
void jsonrpc_session_recv_wait(struct jsonrpc_session *s) {
    if (s->rpc) {
...
bool jsonrpc_session_is_alive(const struct jsonrpc_session *s) {
    return s->rpc || s->stream || reconnect_get_max_tries(s->reconnect);
}

/* Returns true if 's' is currently connected. */
bool jsonrpc_session_is_connected(const struct jsonrpc_session *s) {
    return s->rpc != NULL;
}
...
unsigned int jsonrpc_session_get_seqno(const struct jsonrpc_session *s) {
    return s->seqno;
}
 ...
 * jsonrpc_get_status(). */
int jsonrpc_session_get_status(const struct jsonrpc_session *s) {
...
int jsonrpc_session_get_last_error(const struct jsonrpc_session *s) {
    return s->last_error;
}

/* Populates 'stats' with statistics from 's'. */
void jsonrpc_session_get_reconnect_stats(const struct jsonrpc_session *s,
...
void jsonrpc_session_enable_reconnect(struct jsonrpc_session *s) {
...
void jsonrpc_session_force_reconnect(struct jsonrpc_session *s) {
 ...
 */
void jsonrpc_session_reset_backoff(struct jsonrpc_session *s) {
...
void jsonrpc_session_set_max_backoff(struct jsonrpc_session *s,
                                     int                     max_backoff) {
 ...
 * enabled). */
void jsonrpc_session_set_probe_interval(struct jsonrpc_session *s,
                                        int probe_interval) {
    if (ovs_replay_is_active()) {
        return;
    }
 ...
 * and reconnected. */
void jsonrpc_session_set_dscp(struct jsonrpc_session *s, uint8_t dscp) {
    if (s->dscp != dscp) {
        pstream_close(s->pstream);
        s->pstream = NULL;
...
void jsonrpc_session_set_backlog_threshold(struct jsonrpc_session *s,
                                           size_t                  max_n_msgs,
                                           size_t max_backlog_bytes) {

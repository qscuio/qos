...
static funchook_t *funchook_create_internal(void)
{
	/* 这个handle负责维护所有的全局资源. 维护了一个已分配的page list。每个page包含多个funchook_entry_t. */
...
#else
		/* |funchook|funchook_entry_t|funchook_entry_t|funchook_entry_t|....| */
                                     ...
                                     const funchook_params_t *params)
{
	...
	/* 每个已分配页，可以保存funchook_entry_t，所以优先使用还没有用完的页。*/
...
static int get_page(funchook_t *funchook, funchook_page_t **page_out, uint8_t *addr, ip_displacement_t *disp)
{
	...
	/* 每个page可以保存多个funchook_entry_t, 优先使用没有用完的，如果已经用完的话再分配新的page。 */
    for (page = funchook->page_list; page != NULL; page = page->next) {
	...
	/* 这里根据addr，选择一个离addr最近的还没使用的页，映射进当前进程。*/

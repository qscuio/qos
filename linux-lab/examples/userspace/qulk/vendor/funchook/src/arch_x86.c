...
/*
 * 这个函数的功能是检查两个指针 src 和 dst 之间的距离是否在 32 位有符号整数的范围内，即判断 dst - src 是否在 INT32_MIN 和 INT32_MAX 之间。
 */
...
/*
 * 这个函数的功能是判断在 x86 平台上从 src 到 dst 的跳转是否在 2GB 内。如果可以安全跳转，则返回 1，否则返回 0。
 *
 * 由于 x86 平台上使用相对跳转指令（如 jmp 或 call），跳转指令的长度通常是 5 字节，因此 src + 5 表示跳转指令的结束位置，而不是开始位置。
 * 通过将 src 偏移 5 字节来调用 funchook_within_32bit_relative 函数，可以准确地检查 dst 是否在 32 位相对偏移范围内。
 *
 * 相对跳转指令具有固定的长度（通常为 5 字节），并且需要一个 32 位有符号整数作为偏移量。
 */
...
static int handle_x86_get_pc_by_call_and_pop(make_trampoline_context_t *ctx, const funchook_insn_t *insn)
{
	...
	/* 检查指令是否是 CALL 指令 (0xe8) 并且紧跟的 4 字节（32 位）偏移为 0 */
    if (*ctx->src == 0xe8 && *(uint32_t*)(ctx->src + 1) == 0) {
		/* 计算当前指令地址加上 5 字节，这里假设 funchook_insn_address(insn) 返回当前指令的地址。CALL 指令长度为 5 字节，所以加上 5。*/
        eip = funchook_insn_address(insn) + 5;
		/*
		 * 检查 CALL 指令之后的下一个字节，即 CALL 指令后的第 5 个字节。
		 * 每种 POP 指令会有一个对应的 MOV 指令来替代. 
		 * 这段代码在处理一种特殊情况，即检测到 CALL 指令（目标地址为 0）后紧跟某个 POP 指令时，将其转换为对应的 MOV 指令。
		 */
        switch (*(ctx->src + 5)) {
        case 0x58: /* pop %eax */
            reg_name = "EAX";

...
int funchook_page_alloc(funchook_t *funchook, funchook_page_t **page_out, uint8_t *func, ip_displacement_t *disp)
{
    ...
    for (loop_cnt = 0; loop_cnt < 3; loop_cnt++) {
        ...
        for (i = 1; i >= 0; i--) {
			 ...
			 *
			 * addr: 这是一个建议的映射开始地址，通常设置为 NULL 以让内核选择映射地址。
			 * length: 要映射的内存区域的长度（以字节为单位）。
			 * prot: 内存保护标志，用于指定映射区域的访问权限。常用的保护标志包括：
			 * PROT_READ: 允许读取
			 * PROT_WRITE: 允许写入
			 * PROT_EXEC: 允许执行
			 * PROT_NONE: 不允许访问
			 * flags: 映射标志，用于指定映射对象的类型和映射选项。常用的标志包括：
			 * MAP_SHARED: 共享映射，多个进程对映射区域的修改会互相可见，并会反映到文件中
			 * MAP_PRIVATE: 私有映射，修改对其他进程不可见，也不会反映到文件中
			 * MAP_ANONYMOUS: 匿名映射，不与任何文件关联，通常与 MAP_PRIVATE 一起使用
			 * fd: 打开的文件描述符，通过 open 函数获得。如果使用匿名映射，这个参数设置为 -1。
			 */
...
void *funchook_resolve_func(funchook_t *funchook, void *func)
{
	...
	/*
	 * _r_debug 的来源
	 * _r_debug 通常由 GNU C Library 定义，并在内部使用。例如，在 link.h 中，你会看到以下定义
	 * struct r_debug {
	 *     int r_version;           // Version number for this protocol.
	 *     struct link_map *r_map;  // Head of the chain of loaded objects. 
	 *	   ElfW(Addr) r_brk;        // Address at which the object providing the symbol is loaded.
	 *	   r_state_t r_state;       // The state we are in.
	 *	   ElfW(Addr) r_ldbase;     // Base address the linker is loaded at. 
	 *};
	 * _r_debug 结构体提供了一个链表，描述了当前进程中所有加载的共享对象（shared objects）。这个链表的头部指针是 r_map，它指向一个 link_map 结构体。link_map 结构体包含了每个共享对象的信息，例如基地址、动态段信息等。
	 */

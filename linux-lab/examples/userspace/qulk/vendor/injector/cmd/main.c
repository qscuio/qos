...
int already_injected(pid_t pid, const char *name)
{
	FILE *fp;
...
int main(int argc, char **argv)
{
	...
	const char *optstring = "n:p:r:T";
	int cloned_thread = 0;
#else
	const char *optstring = "n:p:r";
#endif
	...
	if (pid == INVALID_PID) {
		fprintf(stderr, "Usage: %s [-r] [-n process-name] [-p pid] library-to-inject ...\n", argv[0]);
		return 1;
	}
...
#endif
		if (reverse) {
			if (!already_injected(pid, libname)) {
				fprintf(stderr, "\"%s\" is not injected, inject it first\n", libname);
				rv = 1;
			} else {
				if (injector_deinject(injector, libname) == 0) {
					printf("\"%s\" successfully removed\n", libname);
				} else {
					fprintf(stderr, "could not deinject \"%s\"\n", libname);
					fprintf(stderr, "  %s\n", injector_error());
					rv = 1;
				}
			}
			...
			if (already_injected(pid, libname)) {
				fprintf(stderr, "\"%s\" already injected remove it first\n", libname);
				rv = 1;
			} else {

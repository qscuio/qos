...
int bitset_setup(BitSet* bitset, size_t minimum_number_of_bits) {
	...
	// clang-format off
	if (qsc_vector_setup(&bitset->bits,
									 number_of_bytes,
...
int bitset_copy(BitSet* destination, BitSet* source) {
	...
	if (qsc_vector_copy(&destination->bits, &source->bits) == QSC_VECTOR_ERROR) {
		return BITSET_ERROR;
	}
...
int bitset_move(BitSet* destination, BitSet* source) {
	...
	if (qsc_vector_move(&destination->bits, &source->bits) == QSC_VECTOR_ERROR) {
		return BITSET_ERROR;
	}
	destination->size = source->size;
...
int bitset_swap(BitSet* destination, BitSet* source) {
	...
	if (qsc_vector_swap(&destination->bits, &source->bits) == QSC_VECTOR_ERROR) {
		return BITSET_ERROR;
	}
...
int bitset_destroy(BitSet* bitset) {
	if (qsc_vector_destroy(&bitset->bits) == QSC_VECTOR_ERROR) {
		return BITSET_ERROR;
	}
	return BITSET_SUCCESS;
}
...
int bitset_flip(BitSet* bitset) {
	...
	QSC_VECTOR_FOR_EACH(&bitset->bits, iterator) {
...
uint8_t* byte_get(BitSet* bitset, size_t index) {
	...
	return (uint8_t*)qsc_vector_get(&bitset->bits, _byte_index(index));
}
...
const uint8_t* byte_const_get(const BitSet* bitset, size_t index) {
	...
	return (const uint8_t*)qsc_vector_const_get(&bitset->bits, _byte_index(index));
}
...
int bitset_set_all_to_mask(BitSet* bitset, uint8_t mask) {
	...
	QSC_VECTOR_FOR_EACH(&bitset->bits, byte) {
...
int bitset_clear(BitSet* bitset) {
	...
	return qsc_vector_clear(&bitset->bits);
}
...
int bitset_reserve(BitSet* bitset, size_t minimum_number_of_bits) {
	...
	return qsc_vector_reserve(&bitset->bits, BITS_TO_BYTES(minimum_number_of_bits));
}
...
int bitset_grow(BitSet* bitset) {
	...
	/* ERROR/SUCCESS flags are the same */
	return qsc_vector_push_back(&bitset->bits, &empty);
}
...
int bitset_shrink(BitSet* bitset) {
	...
	/* ERROR/SUCCESS flags are the same */
	return qsc_vector_pop_back(&bitset->bits);
}
...
bool bitset_is_initialized(const BitSet* bitset) {
	...
	return qsc_vector_is_initialized(&bitset->bits);
}
...
int _bitset_increment_size(BitSet* bitset) {
	...
	if (bitset->size++ % 8 == 0) {
		uint8_t empty = 0;
		if (qsc_vector_push_back(&bitset->bits, &empty) == QSC_VECTOR_ERROR) {
			return BITSET_ERROR;
		}
	}
